#!/usr/bin/env ruby
# Generate the Xcode project for foo_dsp_clap (standalone, no monorepo deps).
#
# Single target: a foobar2000 DSP component that hosts CLAP audio-effect plugins.
# No FluidSynth; the CLAP headers (third_party/clap-headers) are always on the
# header search path. Adapted from foo_tun_midi's generator (MIT lineage).
...
BUNDLE_ID = "com.foobar2000.foo-dsp-clap"
...
# (e.g. point at the sibling foo_tun_midi_mac/SDK-2025-03-07 that's already built).
SDK_PATH = (ENV['FB2K_SDK_PATH'] && !ENV['FB2K_SDK_PATH'].empty?) ? ENV['FB2K_SDK_PATH'] : "SDK-2025-03-07"
SDK_SEARCH = SDK_PATH.start_with?('/') ? SDK_PATH : "$(PROJECT_DIR)/#{SDK_PATH}"

# Version from src/version.h (single source of truth).
def read_version
  vfile = File.join(File.dirname(__FILE__), '..', 'src', 'version.h')
  File.readlines(vfile).each do |line|
    return $1 if line =~ /#define\s+CLAP_DSP_VERSION\s+"([^"]+)"/
  end
  abort "ERROR: CLAP_DSP_VERSION not found in src/version.h"
end
COMPONENT_VERSION = read_version
_v = COMPONENT_VERSION.split('.').map(&:to_i)
BUILD_NUMBER = (_v[0] - 1) * 10 + _v[1] + 1
...
clap_files = Dir.glob("src/Clap/*.{cpp,h,mm}").map { |f| File.basename(f) }

src_groups = [['Core', core_files], ['UI', ui_files], ['Integration', integration_files], ['Clap', clap_files]]
...
uuid_prefix_header = generate_uuid
uuid_sdk_header = generate_uuid
...
puts "  Clap files: #{clap_files.join(', ')}"
...
pbxproj_content += "\t\t#{uuid_prefix_header} /* Prefix.pch */ = {isa = PBXFileReference; lastKnownFileType = sourcecode.c.h; path = Prefix.pch; sourceTree = \"<group>\"; };\n"
pbxproj_content += "\t\t#{uuid_sdk_header} /* fb2k_sdk.h */ = {isa = PBXFileReference; lastKnownFileType = sourcecode.c.h; path = fb2k_sdk.h; sourceTree = \"<group>\"; };\n"
...
/* Begin PBXGroup section */
		...
		#{uuid_src_group} /* src */ = {
			isa = PBXGroup;
			children = (
				#{uuid_prefix_header} /* Prefix.pch */,
				#{uuid_sdk_header} /* fb2k_sdk.h */,
				#{uuid_core_group} /* Core */,
				#{uuid_ui_group} /* UI */,
				#{uuid_integration_group} /* Integration */,
				#{uuid_clap_group} /* Clap */,
			);
			path = src;
			sourceTree = "<group>";
		};
		...
		#{uuid_debug_config_target} /* Debug */ = {
			...
			buildSettings = {
				...
				LIBRARY_SEARCH_PATHS = (
					...
					"#{SDK_SEARCH}/foobar2000/foobar2000_component_client/build/Release",
					"#{SDK_SEARCH}/foobar2000/shared/build/Release",
					"#{SDK_SEARCH}/pfc/build/Release",
				);
			};
			name = Debug;
		};
