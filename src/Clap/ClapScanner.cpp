...
//  foo_dsp_clap
//
//  Adapted from foo_tun_midi's ClapScanner: same descriptor-only scan, but keeps
//  plugins advertising CLAP_PLUGIN_FEATURE_AUDIO_EFFECT instead of instrument.
//
...
// Read a bundle's descriptors (no instantiation) and append its audio effects.
void inspectBundle(const std::string& bundlePath, std::vector<ClapPluginEntry>& out) {
    ...
    // RTLD_LAZY (not NOW): we only read descriptors. macOS won't unload an
    // Obj-C/JUCE image on dlclose, so each scanned bundle stays resident for the
    // session — keep the per-bundle cost low and rely on the persisted cache.
        ...
        if (factory) {
            ...
            for (uint32_t i = 0; i < n; ++i) {
                ...
                for (const char* const* f = d->features; f && *f; ++f)
                    if (std::strcmp(*f, CLAP_PLUGIN_FEATURE_AUDIO_EFFECT) == 0) { effect = true; break; }
                if (!effect) continue;
                ClapPluginEntry e;
...
void persist() {
    ...
    clap_dsp_config::setConfigString(clap_dsp_config::kKeyClapPluginList, s.c_str());
}

bool loadPersisted() {
    std::string s = clap_dsp_config::getConfigString(clap_dsp_config::kKeyClapPluginList, "");
...
const std::vector<ClapPluginEntry>& clapEffects(bool forceRescan) {
